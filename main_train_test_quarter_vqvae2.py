...
parser.add_argument('--arch',       type=str,   default='quartervqvae',  help='network type: vaeIdsia, vaeIdsiaStn')
...
def train(e):
  ...
  for i, (input, target, template) in enumerate(trainloader):
    ...
    #recon_loss = torch.mean(torch.pow(terms['recon'] - template.detach(), 2))
    recon_loss = reconstruction_function(terms['recon'], template)
    loss = terms['vq_loss'] + recon_loss

    
    print('Epoch:%d  Batch:%d/%d  Total Loss:%08f Recon loss:%08f  VQ loss:%08f'%(e, i, batch_iter, loss.data, recon_loss.data, terms['vq_loss'].data))
   
    f_loss = open(os.path.join(result_path, "log_loss.txt"),'a')
    f_loss.write('Epoch:%d  Batch:%d/%d  Total Loss:%08f Recon loss:%08f  VQ loss:%08f'%(e, i, batch_iter, loss.data, recon_loss.data, terms['vq_loss'].data))
    f_loss.close()
    ...
    if i < 1 and (e%save_epoch == 0):
      ...
      torchvision.utils.save_image(terms['recon'].data, '{}/batch_{}_recon.jpg'.format(out_folder,i), nrow=8, padding=2)
  ...
  if e%save_epoch == 0:
    ...
    with torch.no_grad():
      class_recon = net(class_template)
    
    torchvision.utils.save_image(class_template.data, '{}/templates.jpg'.format(out_folder), nrow=8, padding=2)  
    torchvision.utils.save_image(class_recon['recon'].data, '{}/templates_recon.jpg'.format(out_folder), nrow=8, padding=2)
...
def score_NN(pred, class_feature, label, n_classes):
  ...
  #print(class_feature.shape, n_classes)
  label = label.numpy()
...
def test(e, best_acc):
  ...
  for i, (input, target, template) in enumerate(testloader):
    ...
    sample_correct, sample_all, sample_rank = score_NN(recon['encoded'], class_recon['encoded'], target, n_classes)
    ...
    if i < 1 and (e%save_epoch == 0):
      ...
      torchvision.utils.save_image(input.data, '{}/batch_{}_data.jpg'.format(out_folder,i), nrow=8, padding=2)
      #torchvision.utils.save_image(input_stn.data, '{}/batch_{}_data_stn.jpg'.format(out_folder, i), nrow=8, padding=2) 
      torchvision.utils.save_image(recon['recon'].data, '{}/batch_{}_recon.jpg'.format(out_folder,i), nrow=8, padding=2)
      torchvision.utils.save_image(template.data, '{}/batch_{}_target.jpg'.format(out_folder,i), nrow=8, padding=2)
