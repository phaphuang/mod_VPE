...
from models.vaeIdsiaStn import VAEIdsia
...
class MetaSim:
    ...
    def initialize_models(self):
        ...
        self.metanet_vae = VAEIdsia(nc=3, input_size = 64, latent_variable_size=300, cnn_chn=[100, 150, 250] ,param1=[200,300,200], param2=None, param3 = [150, 150, 150])
        self.metanet_vae.to(device)

        # Construct optimiser
        self.metavae_optim = optim.Adam(self.metanet_vae.parameters(), lr=args.lr) # 1e-4
    ...
    def inner_loop(self, e, data):
        ...
        recon, mu, logvar, input_stn = self.metanet_vae(input)
        vae_loss = loss_function(recon, template, mu, logvar)

        vae_loss.backward()
        self.metavae_optim.step()
        
        return vae_loss.item(), input, recon, template
    ...
    def meta_training_loop(self, e, trainloader):
        ...
        for _ in range(1):
            vae_loss, input, recon, template = self.inner_loop(e, data)
            vae_total_loss += vae_loss
        
        print(f"Episode: {e}\tTotal VAE Loss: {vae_total_loss:.2f}")
