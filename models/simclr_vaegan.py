...
class VAEIdsia(nn.Module):
    def __init__(self, nc, input_size, conditioned_latent_size=64, latent_variable_size=300, cnn_chn=[100, 150, 250], param1=None, param2=None, param3=None):
        ...
        # decoder
        self.d1 = nn.Linear(latent_variable_size+conditioned_latent_size, int(input_size/8*input_size/8*self.cnn_chn[2]))
    ...
    def forward(self, x, z_i):
        ...
        z_con = self.reparametrize(mu, logvar)
        z = torch.cat([z_con, z_i], dim=1)
        res = self.decode(z)
...
#### source: https://github.com/mrzhu-cool/pix2pix-pytorch/blob/master/networks.py
...
class define_D(nn.Module):
    def __init__(self, ch, nf):
        ...
        self.head = nn.Sequential(
            nn.Conv2d(self.ch, self.nf, 3, 1, 1),
            nn.LeakyReLU(0.2)
        )
